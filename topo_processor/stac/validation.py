...
from topo_processor.util.configuration import lds_cache_bucket, lds_cache_read_role
...
def validate_stac(
    ...
    get_log().debug("validate_stac", bucketName=lds_cache_bucket, layerNumber=layer)

    if layer:
        metadata_file = lds_cache.get_layer(layer, version)
