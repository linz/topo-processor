...
def process_directory(source_dir: str, data_type: DataType) -> None:
    ...
    if total_asset == 0:
        get_log().warn("No Assets Found", total_assets=total_asset, source_dir=source_dir, duration=time_in_ms() - start_time)
        return
    ...
    if len(item_store) == 0:
        get_log().warn("No Items Created", total_assets=total_asset, source_dir=source_dir, duration=time_in_ms() - start_time)
        return
